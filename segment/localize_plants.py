...
def main(cfg: DictConfig) -> None:
    ...
    if plant_detdir.exists() and any(plant_detdir.iterdir()):
        ...
        for det in detections:
            ...
            if not all_present:
                continue
                # Create a DataFrame with the items of list1 as columns
                # df = pd.DataFrame(columns=present_columns)
                # Adding an empty row to the DataFrame
                # df.loc[0] = [None for _ in range(len(df.columns))]
        ...
        #     if save_detection:
        #         df, img, imgpath = inference(imgp, model, save_detection=save_detection)
        #         num = 0
        ...
        #             cv2.imwrite(cropsave_path, cv2.cvtColor(crop, cv2.COLOR_BGR2RGB))
        #             num += 1
        #     dfimgs.append(df)
