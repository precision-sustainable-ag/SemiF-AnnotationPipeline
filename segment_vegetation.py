...
from datasets import BanchImages
from semif_utils import (Relabel_Bbox_Extract_Mask_Colors, apply_mask,
                         clean_mask, clear_border, crop_cutouts, get_images,
...
class SegmentVegetation:
    ...
    def __init__(self, projectdir, data, vi_idx, clsalgo, num_cls):
...
def main(cfg: DictConfig) -> None:
    ...
    projectdir = create_projectdir(workdir, projectdir)

    vegdata = VegDataIterator(datadir)

    for i in tqdm(vegdata,
                  total=len(vegdata.imgs),
                  desc="Segmenting Vegetation from detection results"):
        SegmentVegetation(projectdir, i, vi_idx, clsalgo, num_cls)
