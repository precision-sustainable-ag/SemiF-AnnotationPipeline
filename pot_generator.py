...
from synth_utils import overlay
"""
...
class BenchDataset:
    ...
    def __init__(self):
        ...
        self.commonnames = [
            "clover", "cowpea", "goosefoot", "grasses", "horseweed",
    ...
    def _validate_args(self, args):
        ...
        self.mode = args.mode
        if self.mode != 'random':
            assert self.mode_parameter is not None, '--mode by_week or by_commonname requires --mode_parameter.'
        self.mode_parameter = args.mode_parameter
        if args.mode_parameter:
            assert self.mode_parameter.startswith(
                'week'
            ) or self.mode_parameter in self.commonnames, 'mode_parameter error'
        self._validate_input_directory()

    def _validate_input_directory(self):
        # Validate input directories
        self.bench_dir = Path(args.bench_dir)
        assert self.bench_dir.exists(
        ), f'bench directory does not exist: {args.bench_dir}'
        self.pot_dir = Path(args.pot_dir)
        assert self.pot_dir.exists(
        ), f'pot directory does not exist: {args.pot_dir}'
        self.save_dir = Path(args.save_dir)
        if not self.save_dir.exists():
            assert self.save_dir.exists(
            ), f'save directory does not , creating save directory: {args.save_dir}'
            self.save_dir.mkdir(parents=True, exist_ok=True)
        # Create list of week directories
        self.annotation_dir = Path(args.annotation_dir)
        assert self.annotation_dir.exists(
        ), f'annotation directory does not exist: {args.pot_dir}'
    ...
    def _create_foreground_dict(self):
        ...
        if str(self.annotation_dir.parts[-1]).startswith("annotations"):
            for week_dir in self.annotation_dir.iterdir():
                ...
                if not week_dir.is_dir():
                    warnings.warn(
                        f'file found in week directory (expected common name directories), ignoring: {week_dir}'
                    )
                    continue
                ...
                for commonname_dir in sorted(Path(week_dir).iterdir()):
                    ...
                    if not commonname_dir.is_dir():
                        warnings.warn(
                            f'file found in common name directory (expected category directories), ignoring: {commonname_dir}'
                        )
                        continue
                    ...
                    for image_file in sorted(Path(commonname_dir).iterdir()):
                        if not image_file.is_file():
                            warnings.warn(
                                f'a directory was found inside a common name directory, ignoring: {str(image_file)}'
                            )
                            continue
                        ...
                        if category not in self.foregrounds_dict[
                                super_category]:
                            self.foregrounds_dict[super_category][
                                category] = []
                        self.foregrounds_dict[super_category][category].append(
                            image_file)
        if str(self.annotation_dir.parts[-1]).startswith("week"):
            ...
            assert len(
                self.foregrounds_dict) > 0, 'no valid foregrounds were found'
    ...
    def generate_bench_images(
            self,
            mode='random'):  # 'random', 'by_week', 'by_commonname', or 'choas'
        ...
        for i in tqdm(range(self.count)):
            ...
            pot_paths = np.random.choice(self.pot_images,
                                         pot_alignment,
                                         replace=False)
            # Check for mode
            if self.mode == 'random':
                # Get list of pot paths
                rdmn_plant_paths = np.random.choice(plant_paths,
                                                    pot_alignment,
                                                    replace=True)
            ...
            if not save_imagedir.exists():
                print(
                    f'save image directory does not , creating directory: {save_imagedir}'
                )
                save_imagedir.mkdir(parents=True, exist_ok=True)

            if not save_maskdir.exists():
                print(
                    f'save mask directory does not , creating directory: {save_maskdir}'
                )
            ...
            if self.week:
                save_imagepath = Path(save_imagedir,
                                      f'{self.week}_' + file_stem + '.png')
                save_maskpath = Path(save_maskdir,
                                     f'{self.week}_' + file_stem + '.png')
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bench_dir",
                        ...
                        required=True,
                        dest="bench_dir",
                        default="pi/synthetic_bench/bench",
                        help="Location of empty bench images.")
    parser.add_argument("--pot_dir",
                        type=str,
                        required=True,
                        dest="pot_dir",
                        default="pi/synthetic_bench/pots",
                        help="Location of empty pot directory.")
    parser.add_argument("--annotation_dir",
                        type=str,
                        required=True,
                        dest="annotation_dir",
                        default="pi/annotations",
                        help="Location of annotation plants.")
    parser.add_argument("--save_dir",
                        type=str,
                        required=True,
                        dest="save_dir",
                        default="pi/synthetic_bench",
    ...
    parser.add_argument(
        ...
        dest="mode",
        default="random",
        help="'random', 'by_week', 'by_commonname', or 'choas'")
    parser.add_argument(
        "--mode_parameter",
        type=str,
        dest="mode_parameter",
        default=None,
        help=
        "If mode is 'by_week', give week info (ie 'week1'). If mode is 'commonname', give commonname."
    )
    ...
    parser.add_argument(
        ...
        dest="exclude",
        default=None,
        help="Commonname to exclude for the purpose of balancing dataset.")
