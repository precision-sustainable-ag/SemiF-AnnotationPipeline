...
class ImageComposition():
    ...
    """
    def __init__(self):
        self.allowed_output_types = ['.png', '.PNG','.jpg', '.jpeg', '.JPG']
        self.allowed_background_types = ['.png', '.PNG','.jpg', '.jpeg', '.JPG']
        self.zero_padding = 8 # 00000027.png, supports up to 100 million images
        self.max_foregrounds = 3
        # self.mask_colors = get_rgb_lst(self.max_foregrounds)
        self.mask_colors =  [(255, 255, 255),(255, 255, 255),(255, 255, 255)]
                            # (85, 116, 147), (130, 207, 226),(98, 210, 111),
                            # (41, 61, 118), (152, 251, 193), (58, 75, 89),
                            # (10, 69, 214), (254, 34, 44), (119, 226, 74), 
                            # (137, 127, 35), (228, 160, 212), (67, 113, 201)]#, (195, 107, 201), (51, 46, 248), (181, 120, 47), 
                            # (222, 193, 12), (251, 199, 194), (18, 84, 10), (200, 139, 157), (138, 107, 167), (160, 124, 128), (7, 240, 132)]
    ...
    def _validate_and_process_output_directory(self):
        ...
        if not self.silent:
            ...
            for _ in self.images_output_dir.iterdir():
                ...
                should_continue = 'y' #input('output_dir is not empty, files may be overwritten.\nContinue (y/n)? ').lower()
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        composite = composite.resize((512,512), Image.NEAREST)
        ...
        # mywidth = 1028 
        # myheight = round(self.height) 
        # wpercent = (mywidth/float(background.size[0]))
        # hsize = int((float(background.size[1])*float(wpercent)))
        # composite = background.resize((mywidth,myheight), Image.NEAREST)

    
        composite_mask = Image.new('RGB', composite.size, 0)
        composite_mask = composite_mask.resize((512,512), Image.NEAREST)
        ...
        for fg in foregrounds:
            ...
            # center_x = composite.size[1] / 2
            # center_y = composite.size[0] /2
            # max_center_x = center_x + 30
            # min_center_x = center_x - 70
            # max_center_y = center_y + 30
            # min_center_y = center_y - 70
            ...
            # paste_position = (random.randint(min_center_x, max_center_x), random.randint(min_center_y, max_center_y))
            ...
            # composite = composite.resize((512,512), Image.ANTIALIAS)
            ...
            # composite_mask = composite_mask.resize((512,512), Image.ANTIALIAS)
            # composite = composite.resize((512,512), Image.ANTIALIAS)
    ...
    def _transform_foreground(self, fg, fg_path):
        ...
        print('-----------------------------------------------------------------')
        # print(fg_path)
        print("Beginning Shape: ", np.shape(fg_image))
        ...
        scale = random.random() * .1 + .02  # Pick something between .5 and 1
        new_size = (int(fg_image.size[0] * scale), int(fg_image.size[1] * scale))
        
        # If scale makes images largers than given width and height, rescale to smaller size
        if new_size[0] > self.width or new_size[1] > self.height:
            mywidth = 100 
            wpercent = (mywidth/float(fg_image.size[0]))
            hsize = int((float(fg_image.size[1])*float(wpercent)))
            new_size = (mywidth,hsize)
        ...
        fg_image = fg_image.resize(new_size, resample=Image.ANTIALIAS)

        # Adjust foreground brightness
        brightness_factor = random.random() * .4 + .5 # Pick something between .7 and 1.1
        ...
        print("End Shape: ", np.shape(fg_image))
        print('-----------------------------------------------------------------')
        return fg_image
    ...
    def _create_info(self):
        ...
        should_continue = 'y' #input('Would you like to create dataset info json? (y/n) ').lower()
        if should_continue != 'y' and should_continue != 'yes':
...
if __name__ == "__main__":
    ...
    # python /home/admin_mkutugata/Documents/cocosynth/python/image_composition.py --input_dir /home/admin_mkutugata/Documents/Poa/Data/data/train --output_dir /home/admin_mkutugata/Documents/Poa/Data/data/train/outputs --count 100 --width 512 --height 512 "
